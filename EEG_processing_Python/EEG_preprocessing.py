...
RAW_FILES = glob.glob(RAW_FILEPATH + '*.bdf')
...
def pre_pp():
    for file in RAW_FILES:
        ...
        if not exists:
            #imports raw .bdf data (thereby converting to .fif format)
            raw = mne.io.read_raw_edf(file, preload=True)
            #drops channels 65 and up so that only EEG channels remain
            drop_channels = raw.ch_names[64:len(raw.ch_names)]
...
def manual_bads():    
    ...
    if not PRE_BADS_FILES == []:
        ...
        if not exists:
            ...
            raw.plot(n_channels=64, clipping = 'clamp')
...
def pre_ica():
    ...
    for file in PRE_ICA_FILES:
        ...
        exists = os.path.isfile(post_ica_filepath) or os.path.isfile(manual_post_ica_filepath) or os.path.isfile(preprocessed_filepath) or os.path.isfile(manual_preprocessed_filepath)
        if not exists:
           raw = mne.io.read_raw_fif(file, preload=True)
           ...
           ica = mne.preprocessing.ICA(random_state=0, n_components = 64, method='fastica')
           ica.fit(raw)
